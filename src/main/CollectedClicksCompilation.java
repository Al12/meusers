...
public class CollectedClicksCompilation {
    ...
    public static String getMetadata(String name, Result r) {
        return Bytes.toString(r.getValue(Bytes.toBytes("metadata"),
                Bytes.toBytes(name)));
    }

    public static void workWithNextRecord(byte[] row, boolean auto)
            throws IOException {
        boolean hasDecidedUid = false;
        Get g = new Get(row);
        ...
        int size = Bytes.toInt(r.getValue(Bytes.toBytes("metadata"),
                Bytes.toBytes("size")));
        int clicks = Bytes.toInt(r.getValue(Bytes.toBytes("metadata"),
                Bytes.toBytes("clicks")));
        System.out.println(size + " metrics records | " + clicks
                + " valid point-and-clicks");
        String recordMetricsString = Bytes.toString(r.getValue(
                Bytes.toBytes("data"), Bytes.toBytes("metrics")));
        if (!userMetrics.isEmpty()) {
            ...
            for (AngleBasedMetrics metrics : recordMetrics) {
                curvativeAngleDistr.newRecord(metrics.curvativeAngle * 180
                        / Math.PI);
            ...
            System.out.println("Distance to already recorded "
                    + userId
                    + "'s metrics: "
                    + curvativeAngleDistr.distanceTo(userCurvativeAngleDistr)
                    + "|"
                    + curvativeDistanceDistr
                            .distanceTo(userCurvativeDistanceDistr));
        }
        //test        
        if (uid != null) {
            String[] split = recordMetricsString.split(",");
            ...
            UserIdentityChecker checker = new UserIdentityChecker();
            checker.loadABMArray(Bytes.toString(row), recordMetrics);
            if ( checker.checkIdentity(uid) > 0) {
                System.out.println(Bytes.toString(row) + " is decided to be "+uid);
            } else if ( checker.checkIdentity(uid) < 0) {
                System.out.println(Bytes.toString(row) + " is an impostor of "+uid);
            }
        }
        
        boolean answerYes;
        if (hasDecidedUid) {
            answerYes = uid.equals(userId);
            if (answerYes) {
                System.out.println("Saving as " + userId + "'s record");
            }
        } else {
            System.out.println("Is it user " + userId + "'s record? (Y/N)");
            String answer = keyboard.next();
            answerYes = answer.toLowerCase().equals("y");
        }
        if (answerYes) {
            if (!userMetrics.isEmpty()) {
            ...
            for (int i = 0; i < split.length; ++i) {
                ...
                userCurvativeAngleDistr.newRecord(metrics.curvativeAngle * 180
                        / Math.PI);
            ...
            Put p = new Put(row);
            p.add(Bytes.toBytes("metadata"), Bytes.toBytes("uid"),
    ...
    public static void main(String[] args) {
        ...
        if (args.length >= 2) {
            System.err.printf("Usage: %s [generic options] [auto]\n",
                    CollectedClicksCompilation.class.getSimpleName());
            return;
        }
        if (args.length >= 1) {
            modeAuto = (args[0].equals("auto"));
        }
        System.out
                .println("Enter the id of user, whose clicks will be collected:");
        ...
        try {
            ...
            userMetrics = "";
            userCurvativeAngleDistr = new AbmProbabilityDistributionFunction(
                    120, 180, 20);
            userCurvativeDistanceDistr = new AbmProbabilityDistributionFunction(
                    0, 0.35, 20);
            fullSize = 0;
            for (Result rr : scanner) {
                byte[] key = rr.getRow();
                workWithNextRecord(key, modeAuto);
            }
            tableIn.close();
            if (userMetrics.length() > 0) {
                System.out.println("Writing " + fullSize
                        + " collected metrics records to hbase");
                conf = HBaseConfiguration.create();
                tableOut = new HTable(conf, "userclicks");
                byte[] row = Bytes.toBytes(userId);
                Put p = new Put(row);
                p.add(Bytes.toBytes("data"), Bytes.toBytes("metrics"),
                        Bytes.toBytes(userMetrics));
                tableOut.put(p);
                tableOut.close();
            }
        } catch (IOException e) {
